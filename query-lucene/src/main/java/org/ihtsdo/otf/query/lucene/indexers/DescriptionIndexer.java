...
import org.ihtsdo.otf.query.lucene.PerFieldAnalyzer;
    ...
    */
    ...
    private void setupNidConstants() {
        ...
        if (!sequencesSetup.get()) {
            ...
            try {
                if (!sequencesSetup.get()) {
                    sequenceTypeMap.put(IsaacMetadataAuxiliaryBinding.FULLY_SPECIFIED_NAME.getConceptSequence(), LuceneDescriptionType.FSN.name());
                    sequenceTypeMap.put(IsaacMetadataAuxiliaryBinding.DEFINITION_DESCRIPTION_TYPE.getConceptSequence(), LuceneDescriptionType.DEFINITION.name());
                    sequenceTypeMap.put(IsaacMetadataAuxiliaryBinding.SYNONYM.getConceptSequence(), LuceneDescriptionType.SYNONYM.name());
                    descSourceTypeSequence = IsaacMetadataAuxiliaryBinding.DESCRIPTION_SOURCE_TYPE_REFERENCE_SETS.getConceptSequence();
                }
