...
public class ImportEConceptOchreModel implements Callable<Void> {
	...
	private static final SememeBuilderService SEMEME_BUILDER_SERVICE = LookupService.getService(SememeBuilderService.class);
		...
		try {
					...
					switch (id.getIdType()) {
						...
						case STRING:
							SememeBuilder<SememeChronology<MutableStringSememe<?>>> stringIdBuilder
									  = SEMEME_BUILDER_SERVICE.getStringSememeBuilder((String) id.getDenotation().toString(),
			...
			for (TtkDescriptionChronicle desc : eConcept.getDescriptions()) {
				...
				SememeBuilder<? extends SememeChronology<? extends DescriptionSememe>> descBuilder
						  = SEMEME_BUILDER_SERVICE.getDescriptionSememeBuilder(caseSignificanceConceptSequence,
			...
			LOG.error("Failure importing " + eConcept.toString(), e);
			throw e;
		} finally {
	...
	public static void makeSememe(TtkRefexDynamicMemberChronicle member) throws UnsupportedOperationException, IllegalStateException {
		...
		SememeBuilder<SememeChronology<DynamicSememe<?>>> sememeBuilder = SEMEME_BUILDER_SERVICE
		...
		switch (member.getType()) {
			case CID:
				...
				SememeBuilder<SememeChronology<MutableComponentNidSememe<?>>> cidBuilder
						  = SEMEME_BUILDER_SERVICE.getComponentSememeBuilder(Get.identifierService().getNidForUuids(cidMember.uuid1),
			...
			case MEMBER:
				...
				SememeBuilder<SememeChronology<MutableSememeVersion<?>>> mb
						  = SEMEME_BUILDER_SERVICE.getMembershipSememeBuilder(referencedComponentNid,
									 assemblageSequence);
