...
import gov.vha.isaac.cradle.tasks.AddStampOrigin;
...
import gov.vha.isaac.cradle.tasks.ImportEConceptFile;
...
@RunLevel(value = 1)
public class Cradle extends Termstore implements ObjectChronicleTaskService,
		CradleExtensions, IdentifiedObjectService {
	...
	Cradle() throws IOException, NumberFormatException, ParseException {
		conceptMap = new ConcurrentSequenceSerializedObjectMap(
				new ConceptChronicleDataEagerSerializer(), IsaacDbFolder.get()
						.getDbFolderPath(), "concept-map/", ".concepts.map");
	...
	public Stream<ConceptChronicleDataEager> getConceptDataEagerStream(
		...
		return identifierProvider
				...
				.mapToObj(
						(int sequence) -> {
							Optional<ConceptChronicleDataEager> result = conceptMap
									.get(sequence);
							if (result.isPresent()) {
								return conceptMap.get(sequence).get();
							}
							throw new IllegalStateException(
									"No concept for sequence: " + sequence);
						});
	...
	public Stream<ConceptChronicle> getConceptStream() {
		return conceptMap.getStream().map(
				ConceptChronicleDataEager::getConceptChronicle);
	}

	@Override
	public Stream<ConceptChronicle> getParallelConceptStream() {
		return conceptMap.getParallelStream().map(
				ConceptChronicleDataEager::getConceptChronicle);
	}

	@Override
	public Stream<? extends ConceptChronicleBI> getConceptStream(
			ConceptSequenceSet conceptSequences) throws IOException {
		return conceptSequences.stream().mapToObj(
				(int sequence) -> conceptMap.getQuick(sequence)
						.getConceptChronicle());
	}

	@Override
	public Stream<? extends ConceptChronicleBI> getParallelConceptStream(
			ConceptSequenceSet conceptSequences) throws IOException {
	...
	public Task<Integer> startImportLogTask(Path... paths) {
		ImportCradleLogFile importCradleLogFile = new ImportCradleLogFile(
				paths, this);
	...
	@Override
	public void addUncommitted(ConceptChronicleBI conceptChronicleBI)
			throws IOException {
	 ...
	 */
	@Override
	public void addUncommittedNoChecks(ConceptChronicleBI conceptChronicleBI)
			throws IOException {
	...
	@Override
	public void cancel(ConceptChronicleBI conceptChronicleBI)
			throws IOException {
	...
	public void commit(ConceptChronicleBI cc) throws IOException {
		...
		conceptMap.put(sequence,
				(ConceptChronicleDataEager) ((ConceptChronicle) cc).getData());
	}
	...
	@Override
	public void forget(ConceptChronicleBI conceptChronicleBI)
			throws IOException {
		throw new UnsupportedOperationException();
	}
	...
	public NativeIdSetBI isChildOfSet(int parent, ViewCoordinate viewCoordinate) {
		int[] childrenSequences = taxonomyProvider
				.getTaxonomyChildSequencesActive(
						identifierProvider.getConceptSequence(parent),
						viewCoordinate);
		NativeIdSetBI childNidSet = new IntSet();
		IntStream.of(childrenSequences).forEach(
				(sequence) -> childNidSet.add(identifierProvider
						.getConceptNid(sequence)));
		return childNidSet;
	}
	...
	@Override
	public Collection<DbDependency> getLatestChangeSetDependencies()
			throws IOException {
		throw new UnsupportedOperationException();
	}

	@Override
	public int[] getPossibleChildren(int i, ViewCoordinate viewCoordinate)
			throws IOException {
		throw new UnsupportedOperationException();
	}

	@Override
	public TerminologyBuilderBI getTerminologyBuilder(
			EditCoordinate editCoordinate, ViewCoordinate viewCoordinate) {
		return new Builder(editCoordinate, viewCoordinate, this);
	}

	@Override
	public UUID getUuidPrimordialForNid(int nid) throws IOException {
		Optional<UUID> optionalUuid = identifierProvider
				.getUuidPrimordialForNid(nid);
	...
	@Override
	public void putViewCoordinate(ViewCoordinate viewCoordinate)
			throws IOException {
	...
	public NativeIdSetBI isKindOfSet(int kindOfId, ViewCoordinate viewCoordinate) {
		try {
			...
			ConceptSequenceSet sequenceSet = taxonomyProvider
					.getKindOfSequenceSet(
							identifierProvider.getConceptSequence(kindOfId),
							viewCoordinate);
	...
	public boolean hasConcept(UUID cUUID) {
		// first call hasUuid, because this checks if it exists without storing
		// it.
		if (!hasUuid(cUUID)) {
			return false;
		}
		// If we do have a UUID, check if we have a concept. Don't want to call
		// this first, as it permanently stores the UUID as a side effect.
	...
	@Override
	public void addXrefPair(int i, NidPairForRefex nidPairForRefex)
			throws IOException {
	...
	@Override
	public void forgetXrefPair(int referencedComponentNid,
			NidPairForRefex nidPairForRefex) throws IOException {
	...
	@Override
	public int[] getDestRelOriginNids(int i, NidSetBI nidSetBI)
			throws IOException {
		throw new UnsupportedOperationException();
	}
	...
	@Override
	public ConceptChronicleDdo getFxConcept(UUID uuid,
		...
		ConceptChronicleDdo c = new ConceptChronicleDdo(termSnap, chronicle, VersionPolicy.ACTIVE_VERSIONS, RefexPolicy.REFEX_MEMBERS, RelationshipPolicy.ORIGINATING_RELATIONSHIPS);
		return c;
	}

	@Override
	public ConceptChronicleDdo getFxConcept(
			ComponentReference componentReference, UUID viewCoordinateUuid,
	...
	@Override
    public ConceptChronicleDdo getFxConcept(ComponentReference componentReference, 
    		ViewCoordinate viewCoordinate, VersionPolicy versionPolicy, 
    		RefexPolicy refexPolicy, RelationshipPolicy relationshipPolicy) 
    		throws IOException, ContradictionException {
		int nid = componentReference.getNid();
		ConceptChronicleBI chronicle = this.getConcept(nid);
		TerminologySnapshot termSnap = new TerminologySnapshot(this, viewCoordinate);
		ConceptChronicleDdo c = new ConceptChronicleDdo(termSnap, chronicle, versionPolicy, refexPolicy, relationshipPolicy);
		return c;
	}

	@Override
	public ConceptChronicleDdo getFxConcept(UUID uuid, UUID viewCoordinateUuid,
			VersionPolicy versionPolicy, RefexPolicy refexPolicy,
			RelationshipPolicy relationshipPolicy) 
			throws IOException, ContradictionException {
		ConceptChronicleBI chronicle = this.getConcept(uuid);
	...
	@Override
	public void addPropertyChangeListener(CONCEPT_EVENT concept_event,
	...
	@Override
	public void addVetoablePropertyChangeListener(CONCEPT_EVENT concept_event,
	...
	@Override
	public boolean forget(ConceptAttributeVersionBI conceptAttributeVersionBI)
			throws IOException {
		throw new UnsupportedOperationException();
	}

	@Override
	public void forget(DescriptionVersionBI descriptionVersionBI)
			throws IOException {
		throw new UnsupportedOperationException();
	}
	...
	@Override
	public void forget(RelationshipVersionBI relationshipVersionBI)
			throws IOException {
		throw new UnsupportedOperationException();
	}
	...
	@Override
	public void iterateConceptDataInParallel(
			ProcessUnfetchedConceptDataBI processor) throws Exception {
		getParallelConceptDataEagerStream().forEach(
				(concept) -> {
					try {
						processor.processUnfetchedConceptData(concept.getNid(),
								new CradleFetcher(concept));
					} catch (Exception ex) {
						throw new RuntimeException(ex);
					}
				});
	...
	@Override
	public void iterateConceptDataInSequence(
			ProcessUnfetchedConceptDataBI processor) throws Exception {
		getConceptDataEagerStream().forEach(
				(concept) -> {
					try {
						processor.processUnfetchedConceptData(concept.getNid(),
								new CradleFetcher(concept));
	...
	public NativeIdSetBI getAllConceptNids() throws IOException {
		ConcurrentBitSet bitSet = new ConcurrentBitSet(getConceptCount());
		identifierProvider.getConceptSequenceStream().forEach(
				(int sequence) -> {
					bitSet.set(identifierProvider.getConceptNid(sequence));
				});
		return bitSet;
	...
	@Override
	public NativeIdSetBI getComponentNidsForConceptNids(
			NativeIdSetBI conceptNidSet) throws IOException {
		NidSet results = identifierProvider
				.getComponentNidsForConceptNids(conceptNidSet
						.toConceptSequenceSet());
		return new IntSet(results.stream().toArray());
	}

	@Override
	public NativeIdSetBI getConceptNidsForComponentNids(
			NativeIdSetBI nativeIdSet) throws IOException {
		NativeIdSetBI conceptNids = new IntSet();
		IntStream.of(nativeIdSet.getSetValues()).forEach(
				(componentNid) -> conceptNids
						.add(getConceptNidForNid(componentNid)));
		return conceptNids;
	}

	@Override
	public NativeIdSetBI getOrphanNids(NativeIdSetBI nativeIdSetBI)
			throws IOException {
		throw new UnsupportedOperationException();
	}
	...
	public int getAuthorNidForStamp(int stamp) {
		return identifierProvider.getConceptNid(commitManager
	...
	public Status getStatusForStamp(int stamp) {
		return Status
				.getStatusFromState(commitManager.getStatusForStamp(stamp));
	}
	...
	public int getConceptNidForNid(int nid) {
		return identifierProvider.getConceptNid(identifierProvider
				.getConceptSequenceForComponentNid(nid));
	}

	@Override
	public void writeConceptData(ConceptChronicleDataEager conceptData) {
		int sequence = identifierProvider.getConceptSequence(conceptData
				.getNid());
	...
	@Override
	public Collection<RefexMember<?, ?>> getRefexesForAssemblage(
			int assemblageNid) {
		return refexProvider.getRefexesFromAssemblage(assemblageNid).collect(
				Collectors.toList());
	}

	@Override
	public Collection<RefexMember<?, ?>> getRefexesForComponent(int componentNid) {
		return refexProvider.getRefexesForComponent(componentNid).collect(
				Collectors.toList());
	}

	@Override
	public int getStamp(Status status, long time, int authorNid, int moduleNid,
			int pathNid) {
		return commitManager.getStamp(status.getState(), time,
	...
	public ConcurrentSkipListSet<DestinationOriginRecord> getDestinationOriginRecordSet() {
		return ((CradleTaxonomyProvider) taxonomyProvider)
				.getDestinationOriginRecordSet();
	}

	@Override
	public CasSequenceObjectMap<TaxonomyRecordPrimitive> getOriginDestinationTaxonomyMap() {
		return ((CradleTaxonomyProvider) taxonomyProvider)
				.getOriginDestinationTaxonomyRecords();
	}

	@Override
	public Task<Boolean> startVerifyTask(java.nio.file.Path... paths) {
		VerifyLoadEConceptFile loaderTask = new VerifyLoadEConceptFile(paths,
				this);
		ForkJoinPool.commonPool().execute(loaderTask);
		return loaderTask;
	}
	...
	@Override
	public Task<Integer> startLoadTask(ConceptProxy stampPath,
			Path... filePaths) {
		ImportEConceptFile loaderTask = new ImportEConceptFile(filePaths, this,
				stampPath);
		ForkJoinPool.commonPool().execute(loaderTask);
		return loaderTask;
	}
	...
	@Override
	public Task<Boolean> startVerifyTask(ConceptProxy stampPath,
			Path... filePaths) {
		VerifyLoadEConceptFile loaderTask = new VerifyLoadEConceptFile(
				filePaths, this, stampPath);
		ForkJoinPool.commonPool().execute(loaderTask);
		return loaderTask;
	...
	public Task<Integer> startExportTask(ConceptProxy stampPath, Path filePath) {
		ExportEConceptFile exporterTask = new ExportEConceptFile(filePath,
				this, ttkConceptChronicle -> {
	...
	@Override
	public Task<Integer> startLogicGraphExportTask(ConceptProxy stampPath,
			Path filePath) {
	...
	@Override
	public Task<Void> addStampPathOrigin(ConceptProxy stampPath,
			ConceptProxy originStampPath, Instant originTime) {
		AddStampOrigin addStampOrigin = new AddStampOrigin(stampPath,
				originStampPath, originTime, this);
	...
	@Override
	public Task<Integer> startLoadTask(StandardPaths stampPath,
			Path... filePaths) {
		switch (stampPath) {
		case DEVELOPMENT:
			return startLoadTask(IsaacMetadataAuxiliaryBinding.DEVELOPMENT,
					filePaths);
		case MASTER:
			return startLoadTask(IsaacMetadataAuxiliaryBinding.MASTER,
					filePaths);
		default:
	...
	@Override
	public Task<Boolean> startVerifyTask(StandardPaths stampPath,
			Path... filePaths) {
		switch (stampPath) {
		case DEVELOPMENT:
			return startVerifyTask(IsaacMetadataAuxiliaryBinding.DEVELOPMENT,
					filePaths);
		case MASTER:
			return startVerifyTask(IsaacMetadataAuxiliaryBinding.MASTER,
					filePaths);
		default:
			throw new UnsupportedOperationException("Can't handle: "
					+ stampPath);
	...
	public Task<Integer> startExportTask(StandardPaths stampPath, Path filePath) {
		switch (stampPath) {
		case DEVELOPMENT:
			return startExportTask(IsaacMetadataAuxiliaryBinding.DEVELOPMENT,
					filePath);
		case MASTER:
			return startExportTask(IsaacMetadataAuxiliaryBinding.MASTER,
					filePath);
		default:
			throw new UnsupportedOperationException("Can't handle: "
					+ stampPath);
	...
	@Override
	public Task<Integer> startLogicGraphExportTask(StandardPaths stampPath,
			Path filePath) {
		switch (stampPath) {
		case DEVELOPMENT:
			return startLogicGraphExportTask(
					IsaacMetadataAuxiliaryBinding.DEVELOPMENT, filePath);
		case MASTER:
			return startLogicGraphExportTask(
					IsaacMetadataAuxiliaryBinding.MASTER, filePath);
		default:
			throw new UnsupportedOperationException("Can't handle: "
					+ stampPath);
		}
