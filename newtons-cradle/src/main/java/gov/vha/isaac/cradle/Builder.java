...
public class Builder implements TerminologyBuilderBI {
    ...
    public RelationshipChronicleBI construct(RelationshipCAB blueprint) throws IOException, InvalidCAB, ContradictionException {
        ...
        if (relc == null) {
            ...
            cradle.incrementAndGetSequence();
            r.enclosingConceptNid = c.getNid();
            r.nid = cradle.getNidForUuids(blueprint.getComponentUuid());
            cradle.setConceptNidForNid(c.getNid(), r.nid);
            ...
            for (int p : ec.getEditPaths().getSetValues()) {
                if (r.primordialStamp == Integer.MIN_VALUE) {
                    r.primordialStamp
                            = cradle.getStamp(blueprint.getStatus(), Long.MAX_VALUE,
    ...
    public DescriptionChronicleBI construct(DescriptionCAB blueprint) throws IOException, InvalidCAB, ContradictionException {
        ...
        if (desc == null) {
            ...
            d.nid = cradle.getNidForUuids(blueprint.getComponentUuid());
            cradle.setConceptNidForNid(c.getNid(), d.nid);
            ...
            for (int p : ec.getEditPaths().getSetValues()) {
                if (d.primordialStamp == Integer.MIN_VALUE) {
                    d.primordialStamp
                            = cradle.getStamp(blueprint.getStatus(), Long.MAX_VALUE, ec.getAuthorNid(),
                            ec.getModuleNid(), p);
                } else {
                    if (d.revisions == null) {
    ...
    public MediaChronicleBI construct(MediaCAB blueprint) throws IOException, InvalidCAB, ContradictionException {
        ...
        if (imgC == null) {
            ...
            img.nid = cradle.getNidForUuids(blueprint.getComponentUuid());
            cradle.setConceptNidForNid(c.getNid(), img.nid);
    ...
    public ConceptChronicleBI constructIfNotCurrent(ConceptCB blueprint)
        ...
        if (cc == null) {
            ...
            ConceptChronicle concept = cradle.getConcept(cradle.getConceptNid(cc.getNid()));
    ...
    public ConceptChronicleBI construct(ConceptCB blueprint) throws IOException, InvalidCAB, ContradictionException {
        ...
        int cNid = cradle.getNidForUuids(blueprint.getComponentUuid());
        cradle.setConceptNidForNid(cNid, cNid);
